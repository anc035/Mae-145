...
class Node():
    def __init__(self, x, y):
        ...
        self.parent = None
    def __str__(self):
        return " %s %s %s"% (self.x,self.y,self.parent)
...
#         to the tree.
    ...
    def planning(self, animation=True):
        ...
        while self.goal.parent is None:
            ...
            self.new_node = Node(random_point[0], random_point[1])
            indexInList=self.getNearestNode(self.new_node)
    ...
    def calcQnearby(self,node,new_node):
        Qx=.85*node.x+.15*new_node.x
        Qy=.85*node.y+.15*new_node.y
        Q=Node(Qx,Qy)
        return Q
    ...
    def getNearestNode(self,new_node):
        # TODO: implement a method to find the closest node to the random_point
        nodeDists=[]
        for index,node in enumerate(self.node_list):
            nodeDists.append(self.calcDistNodeToPoint(node,new_node))
        return nodeDists.index(min(nodeDists))
